document.addEventListener("DOMContentLoaded", () => {
    ...
    function checkForNumericCode(input) {
        const trimmedInput = input.trim();
        const codeIndex = numericCodes.indexOf(trimmedInput);

        if (codeIndex !== -1) {
            displaySpecialResponse();
            numericCodes.splice(codeIndex, 1); // Remove code after use
